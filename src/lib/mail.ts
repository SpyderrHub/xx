...
export async function sendVerificationEmail(to: string, code: string) {
  ...
  const fromAddress = process.env.EMAIL_FROM || 'QuantisAI <onboarding@resend.dev>';

  const mailOptions = {
    from: fromAddress,
    to,
    subject: 'Verify your QuantisAI account',
    html: `
      <div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px; background-color: #ffffff; border-radius: 24px; border: 1px solid #e2e8f0;">
        <div style="text-align: center; margin-bottom: 32px;">
          <h1 style="margin: 0; color: #FF6600; font-size: 28px; font-weight: 800; letter-spacing: -0.025em;">QuantisAI</h1>
          <p style="margin-top: 8px; color: #64748b; font-size: 14px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.1em;">Vocal Studio Verification</p>
        </div>
        
        <div style="margin-bottom: 32px; color: #1e293b;">
          <p style="font-size: 16px; line-height: 1.6; margin-bottom: 24px;">Welcome to QuantisAI. To activate your account and start generating studio-quality voices, please verify your email address using the code below:</p>
        ...
        <div style="border-top: 1px solid #f1f5f9; pt-24; margin-top: 32px; padding-top: 24px; color: #94a3b8; font-size: 12px; text-align: center; line-height: 1.5;">
          <p style="margin: 0;">If you didn't create an account with QuantisAI, you can safely ignore this email.</p>
          <p style="margin-top: 8px;">&copy; ${new Date().getFullYear()} QuantisAI Systems. All rights reserved.</p>
        </div>
      </div>
    `,
  };
