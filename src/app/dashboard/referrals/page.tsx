      ...
      {!isSubscribed ? (
        <Card className="bg-white/[0.02] border-primary/20 rounded-[2.5rem] overflow-hidden relative group">
          ...
          <CardContent className="p-8 md:p-12 flex flex-col items-center text-center space-y-6">
            <div className="h-16 md:h-20 w-16 md:w-20 rounded-2xl md:rounded-3xl bg-primary/10 flex items-center justify-center border border-primary/20 mb-4 group-hover:scale-110 transition-transform">
              <Lock className="h-8 md:h-10 w-8 md:w-10 text-primary" />
            </div>
            <div className="space-y-2">
              <h2 className="text-xl md:text-2xl font-bold text-white">Unlock Referral Program</h2>
              <p className="text-sm md:text-muted-foreground max-w-sm mx-auto leading-relaxed">
            ...
            <Button asChild size="lg" className="h-12 md:h-14 px-8 md:px-10 rounded-xl md:rounded-2xl bg-primary font-black text-base md:text-lg btn-glow">
          ...
          <div className="lg:col-span-8 space-y-8">
            <Card className="bg-white/[0.02] border-white/5 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden relative shadow-2xl">
              ...
              <CardHeader className="p-6 md:p-10 pb-0">
                <CardTitle className="text-xl md:text-2xl font-black text-white">Your Invitation Link</CardTitle>
              ...
              <CardContent className="p-6 md:p-10 space-y-8 md:space-y-10">
                <div className="space-y-3">
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Input 
                      value={referralLink} 
                      readOnly 
                      className="h-12 md:h-16 bg-white/5 border-white/10 rounded-xl md:rounded-2xl px-4 md:px-6 font-mono text-xs md:text-sm text-primary/80 focus:ring-0"
                    />
                    <Button 
                      onClick={handleCopy}
                      className={cn(
                        "h-12 md:h-16 w-full sm:w-16 shrink-0 rounded-xl md:rounded-2xl transition-all",
                        copied ? "bg-emerald-500 text-white" : "bg-primary text-white hover:bg-primary/90 btn-glow"
                      )}
                    >
                      {copied ? <Check className="h-5 md:h-6 w-5 md:w-6" /> : <Copy className="h-5 md:h-6 w-5 md:w-6" />}
                    </Button>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-3">
                    <Button 
                      ...
                      className="rounded-xl border-white/10 bg-white/5 h-10 md:h-12 gap-2 text-xs md:text-sm hover:bg-green-500/10 hover:border-green-500/20"
                    >
                    ...
                    <Button 
                      ...
                      className="rounded-xl border-white/10 bg-white/5 h-10 md:h-12 gap-2 text-xs md:text-sm hover:bg-sky-500/10 hover:border-sky-500/20"
                    >
                    ...
                    <Button 
                      ...
                      className="rounded-xl border-white/10 bg-white/5 h-10 md:h-12 gap-2 text-xs md:text-sm hover:bg-blue-500/10 hover:border-blue-500/20"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-4 md:p-6 rounded-2xl bg-white/5 border border-white/5 space-y-2">
                    <Zap className="h-4 md:h-5 w-4 md:w-5 text-amber-400" />
                    <h4 className="text-sm md:text-base font-bold text-white">Instant Credit</h4>
                    <p className="text-[10px] md:text-xs text-muted-foreground leading-relaxed">Credits are added to your balance as soon as your referral upgrades.</p>
                  </div>
                  <div className="p-4 md:p-6 rounded-2xl bg-white/5 border border-white/5 space-y-2">
                    <TrendingUp className="h-4 md:h-5 w-4 md:w-5 text-indigo-400" />
                    <h4 className="text-sm md:text-base font-bold text-white">No Cap</h4>
                    <p className="text-[10px] md:text-xs text-muted-foreground leading-relaxed">There is no limit to how many credits you can earn via referrals.</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                <h3 className="text-lg md:text-xl font-bold text-white">Invite History</h3>
                <Badge variant="outline" className="bg-white/5 border-white/10 text-[9px] md:text-[10px] font-black uppercase px-2 md:px-3 py-1">
              ...
              {isRefsLoading ? (
                ...
                <div className="space-y-3">
                  {referrals.map((ref) => (
                    <div key={ref.id} className="p-4 md:p-5 bg-white/[0.02] border border-white/5 rounded-2xl flex items-center justify-between hover:bg-white/[0.04] transition-colors group">
                      <div className="flex items-center gap-3 md:gap-4">
                        <div className="h-8 md:h-10 w-8 md:w-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-black text-[10px] md:text-xs ring-1 ring-primary/20">
                          {ref.referredUserName?.[0] || 'U'}
                        </div>
                        <div>
                          <p className="text-xs md:text-sm font-bold text-white">{ref.referredUserName || 'Anonymous'}</p>
                      ...
                      <div className="text-right">
                        <div className={cn(
                          "flex items-center gap-1.5 text-[9px] md:text-[10px] font-black uppercase mb-1",
                          ref.status === 'completed' ? "text-primary" : "text-muted-foreground"
                        )}>
                          {ref.status === 'completed' ? <Check className="h-2.5 w-2.5 md:h-3 md:w-3" /> : <Loader2 className="h-2.5 w-2.5 md:h-3 md:w-3 animate-spin" />}
                          {ref.status === 'completed' ? 'Verified' : 'Pending'}
                        </div>
                        <p className="text-[9px] md:text-[10px] text-muted-foreground flex items-center gap-1 justify-end">
                ...
                <div className="h-48 border-2 border-dashed border-white/5 rounded-[2rem] flex flex-col items-center justify-center text-center p-8 bg-white/[0.01]">
                  ...
                  <p className="text-xs md:text-sm text-muted-foreground">No referrals recorded yet. Start sharing your link to earn rewards.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <Card className="bg-white/[0.02] border-white/5 rounded-[2rem] overflow-hidden">
              <CardHeader className="p-6 md:p-8 border-b border-white/5 bg-white/[0.01]">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <CardTitle className="text-base md:text-lg font-bold">Network Stats</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="p-6 md:p-8 space-y-6 md:space-y-8">
                <div className="space-y-1">
                  ...
                  <div className="flex items-end gap-1">
                    <span className="text-3xl md:text-4xl font-black text-white">{totalEarned.toLocaleString()}</span>
                    <Badge variant="outline" className="mb-1.5 md:mb-2 border-primary/20 text-primary bg-primary/5 text-[8px] md:text-[9px] uppercase font-black">Chars</Badge>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-lg md:text-xl font-bold text-white">{verifiedReferrals.length}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Conv. Rate</p>
                    <p className="text-lg md:text-xl font-bold text-white">{conversionRate}%</p>
                  </div>
                </div>

                <div className="pt-4 border-t border-white/5">
                  <Button variant="outline" className="w-full h-10 md:h-12 rounded-xl border-white/10 bg-white/5 hover:bg-white/10 font-bold text-[10px] md:text-xs group" asChild>
            ...
            <div className="p-4 md:p-6 rounded-2xl bg-white/[0.02] border border-white/5">
              <div className="flex gap-3 items-start">
                ...
                <p className="text-[9px] md:text-[10px] text-muted-foreground leading-relaxed italic">
