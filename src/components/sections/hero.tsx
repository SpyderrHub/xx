...
const ComparisonCard = () => (
  <motion.div 
    ...
    className="glass-card rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 space-y-4 sm:space-y-6 relative overflow-hidden group shadow-3d border-white/5 backdrop-blur-3xl"
  >
    <div className="absolute top-0 right-0 p-3 sm:p-4">
      <motion.div 
        animate={{ opacity: [0.5, 1, 0.5] }}
        transition={{ duration: 2, repeat: Infinity }}
        className="bg-primary/20 text-primary text-[8px] sm:text-[10px] font-black uppercase tracking-widest px-2 sm:px-3 py-1 rounded-full border border-primary/30"
      >
        Best Value
      </motion.div>
    </div>
    ...
    <div className="space-y-3 sm:space-y-4">
      ...
      <motion.div 
        ...
        className="flex items-center justify-between p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-primary/10 border-2 border-primary/40 shadow-[0_20px_60px_rgba(255,102,0,0.2)] relative group cursor-pointer"
      >
        <div className="absolute inset-0 bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl sm:rounded-3xl" />
        <div className="flex items-center gap-2 sm:gap-3 relative z-10">
          <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-lg sm:rounded-xl bg-primary flex items-center justify-center text-white shadow-lg shadow-primary/30">
            <Zap className="h-4 w-4 sm:h-5 sm:w-5 fill-current animate-pulse" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          >
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-primary text-[7px] sm:text-[10px] font-black uppercase tracking-[0.2em] mb-6 sm:mb-8 shadow-3d backdrop-blur-md"
            >
              <Sparkles className="h-3 w-3 sm:h-3.5 sm:w-3.5 fill-current animate-pulse" />
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-5 w-full sm:w-auto">
              <Link href="/sign-up" className="w-full sm:w-auto">
                <Button size="lg" className="h-12 sm:h-16 w-full rounded-2xl px-8 sm:px-10 text-sm sm:text-lg font-black bg-primary btn-glow shadow-3d hover:scale-105 transition-transform">
                  Get Started Free
                </Button>
              </Link>
          ...
          <motion.div 
            ...
            className="relative lg:ml-10 perspective-1000 hidden sm:block"
          >
            <ComparisonCard />
